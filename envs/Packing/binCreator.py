...
class RandomDeformBoxCreator(BoxCreator):
    ...
    # the range of mass is from 0.1 to 1 (inclusive) kg
    # the range of spring constant is 0.01 to 10.0 (inclusive)
    # the range of fragility index is 0 to 10 (inclusive) (let's assume fragility index == mass * 10)
            ...
            for h in range(4):
                for m in np.arange(0.1, 1.1, 0.1):
                    for k in range(1, 11, 1):
                        default_box_set.append((2 + l, 2 + w, 2 + h, m, k, np.round(m * 10)))
